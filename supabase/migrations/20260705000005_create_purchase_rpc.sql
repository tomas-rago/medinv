-- Atomic order creation: purchase header + line items in one tx, so a
-- failed line insert can never leave an orphan header. security invoker —
-- the writers-manage RLS policies on purchases/purchase_items still apply.
