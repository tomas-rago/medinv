...
export default async function StockServerPage({
  ...
  const { data: movements, count } = await supabase
    ...
    .select("id, type, quantity, expiry_date, notes, created_at, user_id, products(name, category)", {
      count: "exact",
    })
  ...
  return (
    <StockPage movements={rows} count={count ?? 0} page={page} pageSize={PAGE_SIZE} canWrite={canWrite} />
  );
}
