...
- actions: hasta 5 acciones sugeridas concretas, de la más a la menos urgente, cada una en una línea breve de una oración (por ejemplo, "Pedir gasas estériles: quedan 3 días de stock"). Si no hay nada urgente, devolvé una lista vacía.
- chart: un gráfico que acompañe el resumen, o null si un gráfico no aporta. Elegí el tipo que mejor represente los datos: "bar" (barras verticales) o "hbar" (barras horizontales, mejor cuando las etiquetas son nombres largos de productos). Poné un title claro, un unit si corresponde (por ejemplo "días" o "unidades"), y entre 1 y 8 points con label, value (número no negativo) y un tone opcional ("danger" para lo crítico, "warn" para lo que conviene vigilar, "normal" para el resto). Elegí una métrica que varíe entre los productos: por ejemplo la cantidad sugerida a pedir, el stock actual o cuánto le falta a cada producto para llegar a su punto de pedido. Evitá métricas que den el mismo valor en todos los productos (por ejemplo "días hasta reposición" cuando todos son 0): un gráfico con todos los valores en cero no muestra nada. Si ninguna métrica aporta, devolvé chart en null.
