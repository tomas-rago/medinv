...
export function StockEntryModal({ onClose }: StockEntryModalProps) {
  const t = useTranslations("Stock");
  const tCat = useTranslations("Categories");
  const tVal = useTranslations("Validation");
  const tErr = useTranslations("Errors");

  const [state, action, isPending] = useActionState(registerStockEntry, initialState);
    ...
    <div className="mi-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="mi-modal mi-shadow-lg mi-fade" style={{ maxWidth: 540 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5" style={{ maxHeight: "70vh", overflowY: "auto" }}>
          ...
          {/* Entry details — only meaningful once a product is chosen */}
          <form id="stock-entry-form" action={action}>
