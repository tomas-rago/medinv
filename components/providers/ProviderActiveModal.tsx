...
import { toggleProviderActive } from "@/app/(dashboard)/providers/actions";

type ToggleableProvider = {
  id: string;
  name: string;
  active: boolean;
};

interface ProviderActiveModalProps {
  provider: ToggleableProvider;
  onClose: () => void;
}

export function ProviderActiveModal({ provider, onClose }: ProviderActiveModalProps) {
  ...
  const handleToggle = async () => {
    setIsPending(true);
    setErrorKey(null);
    const result = await toggleProviderActive(provider.id, !provider.active);
    setIsPending(false);
    if (result.ok) {
      onClose();
    } else {
  ...
  const actionLabel = provider.active ? t("deactivate") : t("activate");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-display text-ink leading-tight" style={{ fontSize: 18 }}>
                {provider.active ? t("deactivate_title") : t("activate_title")}
              </div>
        ...
        <div className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span
              ...
              style={{ fontSize: 13, fontWeight: 600, color: provider.active ? "var(--c-ok)" : "var(--c-ink-3)" }}
            >
              <span
                style={{
                  width: 8, height: 8, borderRadius: "50%",
                  background: provider.active ? "var(--c-ok)" : "var(--c-ink-3)",
                  flexShrink: 0,
                }}
              />
          ...
          <p className="text-ink2" style={{ fontSize: 14 }}>
            {provider.active ? t("deactivate_confirm") : t("activate_confirm")}{" "}
            <b className="text-ink">{provider.name}</b>?
            {provider.active && (
